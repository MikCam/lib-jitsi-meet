...
export default class XmppConnection extends Listenable {
     ...
     * @private
    */
    _stropheConnectionCb(targetCallback, status, condition, element) {
        this._status = status;
    ...
    send(stanza) {
        if (!this.connected) {
            logger.error(`Trying to send stanza while not connected. Status:${this._status} Proto:${this.isUsingWebSocket ? this._stropheConn?._proto?.socket?.readyState : 'bosh'}`);
     ...
     */
    sendIQ(elem, callback = undefined, errback = undefined, timeout = undefined) {
        if (!this.connected) {
