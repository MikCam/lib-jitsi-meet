...
import { AnalyticsEvents, createBridgeDownEvent, createConnectionStageReachedEvent, createFocusLeftEvent, createJingleEvent, createRemotelyMutedEvent } from './service/statistics/AnalyticsEvents';
...
export default class JitsiConferenceEventManager {
    ...
    setupChatRoomListeners() {
        ...
        chatRoom.addListener(XMPPEvents.DESKTOP_MUTED_BY_FOCUS, (actor) => {
            // TODO: Add a way to differentiate between commands which caused
            // us to mute and those that did not change our state (i.e. we were
            // already muted).
            ...
            conference.rtc.setDesktopMute().then(() => {
                ...
                .catch((error) => {
                conference.mutedDesktopByFocusActor = null;
                logger.warn('Error while desktop video muting due to focus request', error);
            });
        });
        ...
        chatRoom.addListener(XMPPEvents.SESSION_ACCEPT_TIMEOUT, (jingleSession) => {
            Statistics.sendAnalyticsAndLog(createJingleEvent(AnalyticsEvents.ACTION_JINGLE_SA_TIMEOUT, { p2p: jingleSession.isP2P }));
        });
        ...
        chatRoom.addListener(XMPPEvents.SILENT_STATUS_CHANGED, conference.onSilentStatusChanged.bind(conference));
        ...
        (jid, txt, myJid, ts, displayName, isVisitor, messageId, source, replyToId) => {
            const participantId = Strophe.getResourceFromJid(jid);
            conference.eventEmitter.emit(JitsiConferenceEvents.MESSAGE_RECEIVED, participantId, txt, ts, displayName, isVisitor, messageId, source, replyToId);
        });
        this.chatRoomForwarder.forward(XMPPEvents.POLLS_RECEIVE_EVENT, JitsiConferenceEvents.POLL_RECEIVED);
        this.chatRoomForwarder.forward(XMPPEvents.POLLS_ANSWER_EVENT, JitsiConferenceEvents.POLL_ANSWER_RECEIVED);
        chatRoom.addListener(XMPPEvents.REACTION_RECEIVED, (jid, reactionList, messageId) => {
            const participantId = Strophe.getResourceFromJid(jid);
            conference.eventEmitter.emit(JitsiConferenceEvents.REACTION_RECEIVED, participantId, reactionList, messageId);
        });
        chatRoom.addListener(XMPPEvents.PRIVATE_MESSAGE_RECEIVED, 
        // eslint-disable-next-line max-params
        (jid, txt, myJid, ts, messageId, displayName, isVisitor, ofrom, replyToId) => {
            const participantId = isVisitor ? ofrom : Strophe.getResourceFromJid(jid);
            conference.eventEmitter.emit(JitsiConferenceEvents.PRIVATE_MESSAGE_RECEIVED, participantId, txt, ts, messageId, displayName, isVisitor, replyToId);
        });
        ...
        // File sharing
        this.chatRoomForwarder.forward(XMPPEvents.FILE_SHARING_FILES_RECEIVED, JitsiConferenceEvents.FILE_SHARING_FILES_RECEIVED);
        this.chatRoomForwarder.forward(XMPPEvents.FILE_SHARING_FILE_ADDED, JitsiConferenceEvents.FILE_SHARING_FILE_ADDED);
        this.chatRoomForwarder.forward(XMPPEvents.FILE_SHARING_FILE_REMOVED, JitsiConferenceEvents.FILE_SHARING_FILE_REMOVED);
        // Room metadata.
    ...
    setupRTCListeners() {
        ...
        rtc.addListener(RTCEvents.ENDPOINT_MESSAGE_RECEIVED, (from, payload) => {
            if (from === 'transcriber') {
                conference.eventEmitter.emit(JitsiConferenceEvents.NON_PARTICIPANT_MESSAGE_RECEIVED, from, payload);
                return;
            }
    ...
    setupStatisticsListeners() {
        ...
        conference.statistics.addEncodeTimeStatsListener((tpc, stats) => {
            conference.eventEmitter.emit(JitsiConferenceEvents.ENCODE_TIME_STATS_RECEIVED, tpc, stats);
        });
